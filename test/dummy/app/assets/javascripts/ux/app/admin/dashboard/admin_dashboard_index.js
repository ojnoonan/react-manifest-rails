var AdminDashboardIndex = function() {
  ...
  var statusColors = { green: 'bg-green-100 text-green-800', yellow: 'bg-yellow-100 text-yellow-800' };

  return e('div', { className: 'p-6 max-w-6xl mx-auto' },

    e('div', { className: 'flex items-center gap-3 mb-2' },
      e(PageHeader, { title: 'Admin Dashboard', subtitle: 'System overview' }),
      e(Badge, { color: 'red' }, 'Admin')
    ),
    ...
    e('div', { className: 'grid grid-cols-2 md:grid-cols-4 gap-4 mb-10' },
      stats.map(function(s, i) {
        return e('div', { key: i, className: 'bg-white border border-gray-200 rounded-lg p-5' },
          e('div', { className: 'text-2xl mb-2' }, s.icon),
          e('p', { className: 'text-2xl font-bold text-gray-900' }, s.value),
          e('p', { className: 'text-sm text-gray-500 mt-1' }, s.label)
        );
      })
    ),
    ...
    e('div', { className: 'bg-white border border-gray-200 rounded-lg mb-8 overflow-hidden' },
      e('div', { className: 'px-5 py-4 border-b border-gray-100' },
        e('h2', { className: 'font-semibold text-gray-800' }, 'Recent Users')
      ),
      e(DataTable, {
        columns: [
          { key: 'name',   label: 'Name' },
          { key: 'email',  label: 'Email' },
          {
            ...
            label: 'Role',
            render: function(val) {
              return e(Badge, { color: roleColors[val] || 'gray' }, val);
            },
          },
          {
            key: 'joined',
    ...
    e('div', { className: 'bg-white border border-gray-200 rounded-lg overflow-hidden' },
      e('div', { className: 'px-5 py-4 border-b border-gray-100' },
        e('h2', { className: 'font-semibold text-gray-800' }, 'System Status')
      ),
      e('div', { className: 'divide-y divide-gray-100' },
        services.map(function(svc) {
          return e('div', { key: svc.name, className: 'flex items-center justify-between px-5 py-3' },
            e('div', { className: 'flex items-center gap-3' },
              e('span', { className: 'text-lg' }, svc.icon),
              e('span', { className: 'font-medium text-gray-800' }, svc.name)
            ),
            e('span', {
              className: 'text-xs font-semibold px-2.5 py-1 rounded-full ' + (statusColors[svc.color] || 'bg-gray-100 text-gray-700'),
            }, svc.status)
          );
        })
      )
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (container && container.dataset.component === 'AdminDashboardIndex') {
    ReactDOM.createRoot(container).render(React.createElement(AdminDashboardIndex));
  }
});
